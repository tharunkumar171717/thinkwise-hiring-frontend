...
/**
 * Pretty candidate URL: /dashboard/:clientSlug/:reqSlug/:candidateSlug
 * Resolves the slugs back to the requirement id + candidate uuid, then renders
 * the regular candidate full view. Falls back to friendly not-found states.
 */
export default function CandidateSlugView() {
    const { clientSlug, reqSlug, candidateSlug } = useParams<{ clientSlug: string; reqSlug: string; candidateSlug: string }>();
    ...
    const req = requirements.find(
        (r: any) => toSlug(r.company_name || "") === clientSlug && toSlug(r.requirement_name || "") === reqSlug && r.status !== "DELETED"
    ) || requirements.find(
